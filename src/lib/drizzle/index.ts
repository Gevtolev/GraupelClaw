...
export function getDb(): BetterSQLite3Database<typeof schema> {
  if (!_db) {
    ...
    const DATA_DIR =
      process.env.PRIVATE_OPENCLAW_CONSOLE_DATA_DIR ||
      process.env.CHATCLAW_DATA_DIR ||
      join(process.cwd(), "data");
    if (!existsSync(DATA_DIR)) mkdirSync(DATA_DIR, { recursive: true });

    const sqlite = new Database(join(DATA_DIR, projectBrand.storage.sqliteFileName));
    ...
    const migrations = [
      ...
      `ALTER TABLE teams ADD COLUMN workspace_root TEXT`,
    ];
